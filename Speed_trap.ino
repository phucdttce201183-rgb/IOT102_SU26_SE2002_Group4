...
void setup() {
  ...
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print(" LAI XE AN TOAN ");  // Căn giữa, không dấu
  lcd.setCursor(0, 1);
  lcd.print("TRACH NHIEM & XH");  // Viết tắt cho vừa 16 ký tự
...
void loop() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (httpResponseCode > 0) {
      ...
      // CHỈ CẢNH BÁO KHI SERVER TRẢ VỀ CÓ CHỨA CHỮ "WARNING"
      lcd.setCursor(0, 1);
      ...
      } else if (responseText.indexOf("SAFE") >= 0) {
